...
class _AdaptiveSprintScreenState extends State<AdaptiveSprintScreen> {
  ...
  Future<void> _startSprint() async {
    ...
    try {
      ...
      final rows =
          await Supabase.instance.client.rpc('start_adaptive_sprint', params: {
        'p_user_id': userId,
        'p_domain': _domain,
  ...
  Future<void> _submit() async {
    ...
    try {
      final userId = context.read<UserProvider>().currentUser!.uid;
      final result =
          await Supabase.instance.client.rpc('submit_adaptive_sprint', params: {
        'p_user_id': userId,
      ...
      setState(() {
        _stage = _SprintStage.error;
        _error =
            'Could not submit your sprint. Your answers were not saved — try again.';
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: switch (_stage) {
        ...
        _SprintStage.loading => const Center(
  ...
  Widget _buildPicker() => ListView(
        ...
        children: [
          Text('Pick a domain',
              style:
                  GoogleFonts.sora(fontSize: 15, fontWeight: FontWeight.w800)),
                ...
                child: OutlinedButton(
                  ...
                  style: OutlinedButton.styleFrom(
                    backgroundColor: selected
                        ? AppTheme.primaryPurple.withValues(alpha: .1)
                        : null,
                    side: BorderSide(
                        color: selected
    ...
    return Column(children: [
      Padding(
        ...
        child: Row(children: [
          Expanded(
            child: Text(
                'Question ${_currentIndex + 1} of ${_questions.length} · ${question['topic']}',
                style:
                    GoogleFonts.inter(fontSize: 11, color: AppTheme.mutedText)),
          ),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
            decoration: BoxDecoration(
                color: AppTheme.primaryPurple.withValues(alpha: .1),
            ...
            ...List.generate(options.length, (idx) {
              ...
              return Padding(
                ...
                child: InkWell(
                  ...
                  onTap: () =>
                      setState(() => _answers[question['id'].toString()] = idx),
                  child: Container(
                    padding: const EdgeInsets.all(14),
                    decoration: BoxDecoration(
                      color: isSelected
                          ? AppTheme.primaryPurple.withValues(alpha: .06)
                          : Colors.white,
                      borderRadius: BorderRadius.circular(14),
                      border: Border.all(
                          color: isSelected
      ...
      Padding(
        ...
        child: Row(children: [
          ...
          FilledButton(
            style:
                FilledButton.styleFrom(backgroundColor: AppTheme.primaryPurple),
            onPressed: () {
  ...
  Widget _buildReport() {
    ...
    final byDifficulty =
        Map<String, dynamic>.from(report['by_difficulty'] as Map? ?? const {});
    return ListView(
      padding: const EdgeInsets.all(20),
      children: [
        Container(
          ...
          child:
            ...
            if ((report['resolved_revisit_count'] as int? ?? 0) > 0) ...[
              const SizedBox(height: 6),
              Text(
                  '${report['resolved_revisit_count']} previously-missed concept(s) resolved 🎉',
                  style: GoogleFonts.inter(
  ...
  Widget _buildError() => Center(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const SizedBox(height: 16),
            FilledButton(
                onPressed: () => setState(() => _stage = _SprintStage.picker),
