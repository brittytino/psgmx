...
class BirthdayNotificationService {
  ...
  void _scheduleMidnightCheck() {
    ...
    var midnight =
        DateTime(now.year, now.month, now.day + 1, 0, 1); // 12:01 AM next day

    final timeUntilMidnight = midnight.difference(now);

    debugPrint(
        '[BirthdayService] Next midnight check in: ${timeUntilMidnight.inHours}h ${timeUntilMidnight.inMinutes % 60}m');
  ...
  Future<void> _checkBirthdays() async {
    try {
      ...
      debugPrint(
          '[BirthdayService] 🎂 ========================================');
      debugPrint(
          '[BirthdayService] 🎂 Checking birthdays for ${now.year}-${now.month.toString().padLeft(2, '0')}-${now.day.toString().padLeft(2, '0')}');
      debugPrint(
          '[BirthdayService] 🎂 ========================================');
