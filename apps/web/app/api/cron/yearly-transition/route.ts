...
export async function POST(req: NextRequest) {
  ...
  const body = await req.json().catch(() => null) as {
    graduating_batch?: unknown; incoming_batch?: unknown; new_pr?: unknown; outgoing_pr?: unknown;
  ...
  const outgoingPr = typeof body?.outgoing_pr === 'string' ? body.outgoing_pr.trim() : ''
  ...
  const { data, error } = await (supabase.rpc.bind(supabase) as unknown as HandoverRpc)('handover_placement_rep', {
    p_outgoing_batch_code: outgoing, p_incoming_batch_code: incoming, p_incoming_identity: newPr, p_outgoing_identity: outgoingPr,
  })
