    ...
    <div className="flex h-screen w-full bg-page-bg font-sans overflow-hidden relative transition-colors duration-300">
      ...
      >
        ...
        <div className={`flex items-center px-6 h-[80px] shrink-0 border-b border-gray-100 relative ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
            ...
            <button 
              onClick={toggleSidebar}
              className="absolute -right-3 top-1/2 -translate-y-1/2 p-1.5 bg-white border border-gray-200 shadow-md rounded-full z-50 text-[#6C3DFF] hover:bg-[#6C3DFF] hover:text-white transition-all duration-300"
        ...
        <nav className="flex-1 space-y-3 flex flex-col px-6 pt-6 overflow-x-hidden custom-scrollbar">
            {sidebarItems.map((item, index) => {
              ...
              const activeClass = isActive
                ? 'bg-[#6C3DFF] text-white shadow-sm'
            ...
            {!isCollapsed && (
              <div className="bg-purple-50/20 rounded-2xl p-4 border border-purple-100/40 flex flex-col gap-3 w-full shadow-sm">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 shrink-0 flex items-center justify-center bg-purple-50 rounded-xl shadow-sm border border-purple-100/60 text-[#6C3DFF] mt-0.5">
                  ...
                  <div className="flex flex-col min-w-0">
                    {userType === 'Assessments' || userType === 'Candidate' ? (
                      <>
                        ...
                        <span className="text-[9px] font-black text-[#6C3DFF] uppercase tracking-widest leading-relaxed" title={displayInstitutionName}>
                ...
                {userType === 'Campus Partner' && (
                  <button className="w-full py-2.5 rounded-xl border border-purple-100 bg-purple-50/40 hover:bg-purple-50/80 text-[#6C3DFF] flex items-center justify-center gap-2 text-[9px] font-black uppercase tracking-widest transition-all shadow-xs group">
                    <span className="w-3.5 h-3.5 rounded-full bg-[#6C3DFF] text-white flex items-center justify-center text-[8px] font-bold group-hover:scale-110 transition-transform">✓</span>
        ...
        <FloatingElements mode="dashboard" density="low" />
             ...
             <div className="flex items-center gap-3">
                 ...
                 {showBackButton && (
                   <button 
                      ...
                      className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-border-light text-[10px] font-black text-text-muted hover:text-[#6C3DFF] hover:border-red-100 hover:bg-red-50/30 transition-all uppercase tracking-widest shadow-sm group"
                   >
         ...
         <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-zinc-100 z-50 flex items-center shadow-[0_-15px_50px_-15px_rgba(0,0,0,0.15)] pb-safe-offset-1 h-[72px]">
            ...
            <button 
              ...
              className="w-10 h-full flex items-center justify-center text-zinc-400 hover:text-[#6C3DFF] active:scale-90 transition-all bg-gradient-to-r from-white to-transparent z-10"
            >
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center gap-1 transition-all duration-300 relative shrink-0 w-1/4 h-full ${
                      isActive ? 'text-[#6C3DFF]' : 'text-zinc-400'
                    }`}
                  >
                    ...
                    {isActive && (
                      <span className="absolute bottom-1.5 left-1/2 -translate-x-1/2 w-1 h-1 bg-[#6C3DFF] rounded-full shadow-[0_0_8px_#6C3DFF]" />
                    )}
                  </button>
                );
              })}
            ...
            <button 
              ...
              className="w-10 h-full flex items-center justify-center text-zinc-400 hover:text-[#6C3DFF] active:scale-90 transition-all bg-gradient-to-l from-white to-transparent z-10"
            >
