...
import { ArrowRight, Heart, Loader2, Lock, Mail, ShieldAlert, ShieldCheck } from 'lucide-react'
...
function LoginForm() {
  ...
  async function submitStudent(event: React.FormEvent) {
    ...
    try {
      ...
      else window.location.href = (redirectTo && redirectTo !== '/app') ? redirectTo : (result.redirect || '/student')
  ...
  async function submitStaff(event: React.FormEvent) {
    ...
    try {
      ...
      const result = await response.json()
      if (!response.ok) return setError(result.error ?? 'Invalid staff credentials.')
      window.location.href = (redirectTo && redirectTo !== '/app') ? redirectTo : (result.redirect || '/faculty')
    } catch { setError('Network error. Check your connection and try again.') }
    finally { setLoading(false) }
  }
    ...
    <div className="mx-auto flex min-h-[680px] w-full max-w-[1160px] overflow-hidden rounded-[32px] bg-white p-3 shadow-[0_24px_80px_-24px_rgba(16,24,40,.18)]">
      ...
      <section className="flex w-full items-center px-6 py-12 sm:px-14 lg:w-1/2">
        <div className="mx-auto w-full max-w-[420px]">
          ...
          <p className="mt-2 text-sm text-[#667085]">Use the personal or college email approved on your roster.</p>

          <div className="mt-7 grid grid-cols-2 rounded-xl bg-[#F2F4F7] p-1 text-sm font-bold">
            <button onClick={() => { setMode('student'); setError('') }} className={`rounded-lg px-3 py-2.5 ${mode === 'student' ? 'bg-white text-[#FF5A1F] shadow-sm' : 'text-[#667085]'}`}>Student OTP</button>
            <button onClick={() => { setMode('staff'); setOtpSent(false); setError('') }} className={`rounded-lg px-3 py-2.5 ${mode === 'staff' ? 'bg-white text-[#FF5A1F] shadow-sm' : 'text-[#667085]'}`}>Faculty / HOD</button>
          </div>
          ...
          <form onSubmit={mode === 'student' ? submitStudent : submitStaff} className="mt-6 space-y-4">
            <label className="block"><span className="mb-1.5 block text-sm font-bold text-[#101828]">Email address</span><div className="relative"><Mail className="absolute left-4 top-4 h-4 w-4 text-[#98A2B3]" /><input type="email" required value={email} disabled={otpSent} onChange={(e) => setEmail(e.target.value.toLowerCase())} placeholder="you@example.com" autoComplete="email" className="w-full rounded-xl border border-[#D0D5DD] py-3.5 pl-11 pr-4 text-sm outline-none focus:border-[#FF5A1F] disabled:bg-[#F9FAFB]" /></div></label>
            {mode === 'student' && otpSent && <label className="block"><span className="mb-1.5 block text-sm font-bold">Six-digit code</span><input autoFocus inputMode="numeric" pattern="[0-9]{6}" maxLength={6} required value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, ''))} placeholder="000000" className="w-full rounded-xl border border-[#D0D5DD] px-4 py-3.5 text-center text-xl font-black tracking-[.45em] outline-none focus:border-[#FF5A1F]" /></label>}
            {mode === 'staff' && <label className="block"><span className="mb-1.5 block text-sm font-bold">Password</span><div className="relative"><Lock className="absolute left-4 top-4 h-4 w-4 text-[#98A2B3]" /><input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="current-password" className="w-full rounded-xl border border-[#D0D5DD] py-3.5 pl-11 pr-4 text-sm outline-none focus:border-[#FF5A1F]" /></div></label>}
            <button disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#FF5A1F] py-3.5 text-base font-bold text-white shadow-[0_8px_24px_rgba(255,90,31,.25)] transition hover:bg-[#E04812] disabled:opacity-50">{loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <>{mode === 'student' ? otpSent ? 'Verify and continue' : 'Send secure code' : 'Enter staff console'}<ArrowRight className="h-4 w-4" /></>}</button>
            {mode === 'student' && otpSent && <button type="button" onClick={() => { setOtpSent(false); setOtp(''); setMessage('') }} className="w-full text-sm font-bold text-[#667085]">Use a different email</button>}
          </form>
