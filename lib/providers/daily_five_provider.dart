...
class DailyFiveProvider with ChangeNotifier {
  ...
  Future<void> loadState(String userId) async {
    ...
    try {
      ...
      if (_streak == null || !_streak!.completedToday) {
        final questions = await _service.fetchDailyQuestions();
  ...
  /// Called when the app goes to background mid-quiz (anti-cheat).
  /// Marks all unanswered questions as -1 (incorrect) and finalizes.
  Future<void> handleViolation(String userId) async {
    if (_session == null || sessionFinished) return;

    final answers = List<int?>.from(_session!.selectedAnswers);
