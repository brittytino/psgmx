-- ============================================================
-- PSGMX — 03_triggers.sql
-- Database triggers for automatic Edge Function invocation
-- and data integrity maintenance.
-- ============================================================
-- Requires:
--   - pg_net extension (for http_post)
--   - 02_functions.sql (for get_batch_for_user, etc.)
--   - Edge functions deployed and URLs set as DB settings:
...
-- ──────────────────────────────────────────────────────────────
-- TRIGGER HELPER: call_compute_readiness_score(user_id)
-- Calls the compute-readiness-score Edge Function via HTTP POST
-- using pg_net. Fire-and-forget (async).
-- ──────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Read the Edge Function URL from database settings
  -- (Set with: ALTER DATABASE postgres SET app.edge_function_url_compute_readiness_score = '...')
...
-- ──────────────────────────────────────────────────────────────
-- TRIGGER HELPER: call_knowledge_ingest(entry_id)
...
-- Example: '25MX223' matches '24MX223', '23MX223', etc.
-- ──────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Extract the suffix: everything after the first 2 digits + 'MX'
  -- e.g. '25MX223' → suffix = 'MX223'
  -- This matches the pattern [digits]MX[suffix]
  v_roll_suffix := regexp_replace(NEW.roll_no, '^\d+', '');
  -- v_roll_suffix is now 'MX223'

  -- Find all seniors whose roll_no ends with the same suffix
  -- but are NOT in the same batch
  FOR v_senior IN
    SELECT u.id AS senior_id
    FROM users u
    WHERE u.roll_no IS NOT NULL
      AND u.roll_no LIKE '%' || v_roll_suffix
      AND u.roll_no != NEW.roll_no        -- not the same user
      AND u.batch_id != NEW.batch_id      -- different batch
      AND u.batch_id IS NOT NULL
      -- Verify the senior's batch started before the new user's batch
      AND EXISTS (
        SELECT 1 FROM batches b_senior, batches b_junior
        WHERE b_senior.id = u.batch_id
          AND b_junior.id = NEW.batch_id
          AND b_senior.start_date < b_junior.start_date
      )
  LOOP
    -- Insert lineage relationship (junior=new user, senior=found user)
...
AS $$
BEGIN
  -- When role transitions to 'alumni', ensure mentorship_open is reset to false
